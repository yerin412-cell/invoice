...
# --- [1. 정보 입력 영역] ---
st.header("1. 정보 입력 (v2.0)")
client = st.text_input("🏢 거래처명", key="client_v20")
...
if st.button("🚀 명세서 이미지 만들기", type="primary", use_container_width=True):
    if not st.session_state.my_items: st.warning("내역을 먼저 추가해주세요!")
        ...
        try:
            ...
            H_FOOT_START = 850 # 합계 부분 시작 (원본 이미지 기준)

            # 1. 원본 조각 추출
            header = orig.crop((0, 0, W, H_TOP))
            row_gray = orig.crop((0, 345, W, 407))   # 홀수 (회색)
            row_white = orig.crop((0, 407, W, 469))  # 짝수 (흰색, 407+62)
